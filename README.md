...
A search bar for React Native. It has a built in simple search capability which can be utilised by handing the component `data` and providing a `handleResults` function which will receive the filtered copy of the `data` array.
...
![rn-searchbar](http://i.imgur.com/i21YwnW.gif)

(Rendered with the example in these docs and repository)

## Installation

- `npm install https://github.com/mdmush/rn-searchbar --save`
...
- Now you can require the search bar with  `import SearchBar from 'rn-searchbar'` or `var SearchBar = require('rn-searchbar')`
...
Contributing to `rn-searchbar` is easy! With four simple steps:
